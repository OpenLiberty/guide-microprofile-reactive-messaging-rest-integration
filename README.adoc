...
Learn how to integrate RESTful Java microservices with a reactive system using MicroProfile Reactive Messaging.

== What you'll learn

You will learn how to integrate RESTful Java microservices with a reactive system using MicroProfile Reactive
Messaging. RESTful Java microservices do not use reactive concepts, so you will learn how to bridge the gap between the
two using the RxJava library. You will write microservices so that when a user hits the RESTful endpoint, producer events
are generated. You will also learn how we can achieve code execution on demand in reactive applications.
...
microservice. You will also update the `system` microservice to handle receiving and sending events produced by the
new endpoint. You will configure new channels to handle the events sent and received by the new endpoint.
...
Add the RESTful endpoint to the `inventory` service by replacing the `InventoryResource` class.
...
The changes include creating the [hotspot=flowableEmitterDecl file=0]`propertyNameEmitter` variable, and the
[hotspot=updateSystemProperty file=0]`updateSystemProperty()` and [hotspot=sendPropertyName file=0]`sendPropertyName()`
methods. The `updateSystemProperty()` method creates the [hotspot=putPath file=0]`/data` endpoint that
accepts [hotspot=annotatedPut file=0]`PUT` requests with a system property name in the request body.

The `propertyNameEmitter` is a RxJava Emitter which is used to send the property name request to the event stream, Apache
Kafka in this case. The [hotspot=sendPropertyName file=0]`sendPropertyName()` method contains the
[hotspot=flowableCreate file=0]`Flowable.create()` RxJava method which is used to associate the created Emitter to a Publisher
that will be responsible for publishing the events to the event stream. The Publisher in this example is then connected to the
...
The [hotspot=flowableCreate file=0]`Flowable.create()` method also allows the configuration of a
[hotspot=flowableCreate file=0]`BackpressureStrategy` which controls what the Publisher will do if the emitted events can
not be consumed by the subscriber. in this example, the Publisher used the `BackpressureStrategy.BUFFER` strategy, which
will let the Publisher buffer the event until the Subscriber can consume them.
...
The property name sent to the emitter will be sent to the Publisher, which then sends the event to the event channel,
utilizing the configured `BackpressureStrategy` when necessary.
...
incoming and outgoing channels, such as the name of the channel, the topic in the Kafka messaging system, and the associated
message serializers and deserializers. To complete the message loop created in the previous sections, four more channels
...
and `inventory` microservices. Replacing the `inventory` microservice [hotspot file=0]`microprofile-config.properties`
file adds the two channels,
[hotspot=requestSystemProperty file=0]`requestSystemProperty` and [hotspot=addSystemProperty file=0]`addSystemProperty`
...
Replace the `system` microservice [hotspot file=1]`microprofile-config.properties` file to add the two new
[hotspot=propertyRequest file=1]`propertyRequest` and [hotspot=propertyResponse file=1]`propertyResponse`
...
[hotspot=group4 file=1]`group.id` for all of the instances.
