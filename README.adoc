...
:page-description: Learn how to send and receive messages in RESTful microservices using MicroProfile Reactive Messaging.
...
:page-seo-title: Sending and receiving events in RESTful Java microservices
:page-seo-description: A getting started tutorial with examples on how to send and receive messages in RESTful Java applications using Eclipse MicroProfile Reactive Messaging API to integrate with reactive systems or event streaming platforms like Apache Kafka.
...
Learn how to send and receive messages in RESTful Java microservices using MicroProfile Reactive Messaging API to integrate with reactive system.
...
services so that requests are non-blocking and decoupled from responses.
...
By default, the `inventory` microservice records and stores the average system load information from all of the connected
system microservices. Complete the `inventory` service by creating the `InventoryResource` class. The `InventoryResource`
class is also configured to add a RESTful endpoint that, when hit, sends a property request to all of the `system`
microservices to add the selected property information back to the `inventory` service.
...
The newly created RESTful endpoint requires a new channel to be created to move the requested messages between the `system`
and `inventory` microservices. Create the `inventory` microservice [hotspot file=1]`microprofile-config.properties`
configuration, which is already configured with the newly added channels,
...
The `system` microservice [hotspot file=0]`microprofile-config.properties` file is similarly configured with the two new
[hotspot=propertyResponse file=0]`propertyResponse` and [hotspot=propertyRequest file=0]`propertyRequest`
channels.
