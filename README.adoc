...
You can use a `POST` request on the `\http://localhost:9085/inventory/systems` URL to add the value of a particular system property to the set of existing properties.

For example :

The `POST` request with `os.name` in the request body on `\http://localhost:9085/inventory/systems` URL will add the `os.name` system property for your system.
...
[role=command]
```
curl -X POST -d "os.name" http://localhost:9085/inventory/systems --header "Content-Type:text/plain"
```
--
