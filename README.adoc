...
The `system` microservice contains a method named [hotspot=sendPropertyDetails file=0]`sendProperty()`. It receives a
system property name from the `inventory` microservice over the [hotspot=getProperty file=0]`@Incoming("propertyRequest")`
channel, calculates the requested property on the fly and publishes it back to Kafka over the
[hotspot=setProperty file=0]`@Outgoing("propertyResponse")` channel. The [hotspot=sendPropertyDetails file=0]`sendProperty()`
...
The messages are transported between the service and the Kafka messaging system through the channels called
[hotspot=systemLoad file=1]`systemLoad` and [hotspot=setProperty file=1]`propertyResponse`.
...
channel. The configuration for this channel is defined in the
...
microservice creates a system property request when a property name is sent in a PUT request to the [hotspot=postPath file=0]`/data`
endpoint. The [hotspot=getSystemProperty file=0]`updateSystemProperty()` method receives a system property name like `os.name`
...
[hotspot=setProperty file=1]`requestSystemProperty` stream and MicroProfile Reactive Messaging takes care of assigning the
...
and [hotspot=setProperty file=0]`propertyResponse` channel, to the [hotspot=topic1 file=0]`systemLoadTopic` and
