...
accepts `PUT` requests with a system property name in the request body. The [hotspot=sendPropertyName file=0]`sendPropertyName()`
method contains the [hotspot=flowableCreate file=0]`Flowable.create()` RxJava method which is used to create a Publisher
and an associated emitter.
