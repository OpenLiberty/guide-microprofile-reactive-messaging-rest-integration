...
You can use a `POST` request on the `\http://localhost:9085/inventory/systems/property/{propertyName}` URL to add the value of a particular system property to the set of existing properties.

For example :

The `POST` request on the `\http://localhost:9085/inventory/systems/property/os.name` URL will add the `os.name` system property for your system.
