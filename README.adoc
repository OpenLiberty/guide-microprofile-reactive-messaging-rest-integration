...
Learn how to send and receive messages in RESTful Java microservices using MicroProfile Reactive Messaging API to integrate with reactive systems.
...
microservice. You will also update the `system` microservice to handle receiving and sending messages produced by the
new endpoint. You will configure new channels to handle the messages sent and received by the new endpoint.
...
== Adding the REST endpoint to produce messages
...
[hotspot=updateSystemProperty file=0]`updateSystemProperty()` and [hotspot=sendPropertyName file=0]`sendPropertyName()` methods.
The `updateSystemProperty()` method creates the [hotspot=putPath file=0]`/data` endpoint that accepts `PUT` requests
with a system property name in the request body.
...
== Adding the property message processor
