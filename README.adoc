...
that it received from potentially multiple instances of the `system` service. Currently, it is consuming messages from the
continuous stream of messages produced by the `system` service. Update the `inventory` service to add an endpoint that will
...
Add the [hotspot=updateSystemProperty file=0]`updateSystemProperty` method that creates a requestable endpoint for reactive message production.

The `inventory` microservice creates a system property request when a property name is sent in a PUT request body to the
[hotspot=putPath file=0]`/data` endpoint. The [hotspot=updateSystemProperty file=0]`updateSystemProperty()` method receives
a system property name like `os.name` from the PUT request body and adds it to the [hotspot=flowableEmitter file=0]`propertyNameEmitter`
