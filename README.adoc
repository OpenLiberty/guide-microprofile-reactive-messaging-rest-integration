...
:page-seo-title: Producing messages using a RESTful endpoint in a reactive Java microservice
...
services so that requests are non-blocking and decoupled from responses. You can learn more about reactive Java microservices
...
[hotspot=flowableEmitterDecl file=0]`FlowableEmitter`. The FlowableEmitter from RxJava is an observable that emits
objects asynchronously. This bridges the gap between the imperative synchronous
programming portion of the application and the MP Reactive Messaging asynchronous portion.
The [hotspot=flowableCreate file=0]`Flowable.create()` method from
...
[source, Java, linenums, role='code_column hide_tags=copyright']
----
include::finish/models/src/main/java/io/openliberty/guides/models/PropertyMessage.java[]
----
...
microservice can consume the messages. The [hotspot=SystemLoadSerializer file=2]`SystemLoadSerializer` and
[hotspot=PropertyMessageSerializer file=3]`PropertyMessageSerializer` classes implements the
...
The [hotspot=SystemLoadDeserializer file=2]`SystemLoadDeserializer` and
[hotspot=PropertyMessageDeserializer file=3]`PropertyMessageDeserializer` classes implements the logic for turning JSON into a `SystemLoad`
...
The `inventory` microservice also publishes messages through the Outgoing channel,
...
Wait for a minute or two for the services to become available. After the services are up and running, you can access the
...
You can make a `PUT` request on the `\http://localhost:9085/inventory/data` URL to add the value of a particular system
