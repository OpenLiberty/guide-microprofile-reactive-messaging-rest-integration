...
The `updateSystemProperty()` method creates the [hotspot=putPath file=0]`/data` endpoint that accepts
...
ifdef::cloud-hosted[]
You can find the **inventory** microservice at **http://localhost:9085/inventory/systems** by running the following curl command:
```
...
// cloud-hosted guide instructions:
ifdef::cloud-hosted[]
You can revisit the **inventory** service after a while, and you will notice the CPU `systemLoad`
property for the systems changed. Run the following curl command to see the change:
```
curl http://localhost:9085/inventory/systems
```
{: codeblock}
