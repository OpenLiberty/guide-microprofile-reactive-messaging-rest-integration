...
The `system` microservice contains a method named [hotspot=sendPropertyDetails file=0]`sendProperty()`. It receives a system property name from the `inventory` microservice over the [hotspot=getProperty file=0]`@Incoming("getProperty")` channel, calculates the requested property on the fly and publishes it back to Kafka over the [hotspot=setProperty file=0]`@Outgoing("setProperty")` channel. The [hotspot=sendPropertyDetails file=0]`sendProperty()` method acts as a processor in this scenario.
...
You can use a `POST` request on the `\http://localhost:9085/inventory/systems/property` URL to add the value of a particular system property to the set of existing properties.

For example :

The `POST` request with `os.name` in the request body on `\http://localhost:9085/inventory/systems/property` URL will add the `os.name` system property for your system.
...
[role=command]
```
curl -X POST -d "os.name" http://localhost:9085/inventory/systems/property --header "Content-Type:text/plain"
```
--
...
into the URL bar and change the request from `GET` to `POST`. Go to the `Body` tab and enter the `os.name` value under raw category. Click the blue `Send` button to make the request.
