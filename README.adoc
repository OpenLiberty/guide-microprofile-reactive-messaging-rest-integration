...
The `system` microservice contains a method named [hotspot=sendPropertyDetails file=0]`sendProperty`. It receives a system property name from the `inventory` microservice over the [hotspot=getProperty file=0]`@Incoming("getProperty")` channel, calculates the requested property on the fly and publishes it back to the Kafka over the [hotspot=setProperty file=0]`@Outgoing("setProperty")` channel. The [hotspot=sendPropertyDetails file=0]`sendProperty` method acts as a processor in this scenario.
